...
\begin{equation}
  ...
  \label{eq:drag1}
\end{equation}
$\Omega_1$ has to satisfy the boundary conditions $\, \Omega_1 = 0 \text{ at }t=0,\,T. $
...
In the same spirit, applying SWTs (See \ref{appendix:DRAG2}) removes
$\ket{0}\leftrightarrow\ket{3}$ transition and
\begin{equation}
  \Omega_x(t) = \sqrt{\Omega_1^2 + \frac{2}{\Delta_2^2} (\dot{\Omega}_1^2 + \ddot{\Omega}_1 \Omega_1)}.
  \label{eq:drag2}
\end{equation}
...
that distinctly separates the computational subspace (principla $2\times 2$ block) from the non-computational.
...
the specific unitary transformation $U(t)$ that achieves this decoupling is constructed as,
$$
