...
The validity of this particular solution can be analytically verified with
the Leibniz rule \cite{apostol}, which dictates the differentiation under the integral sign:
\begin{equation}
...
The differential DRAG-1 formula \eref{eq:drag1} implies
$$
...
we now utilize echo method to create pulses.
\subsubsection{DRAG-1 pulse}
\label{subsubsec:drag1_method}
We propose a method to create a flat-top DRAG-1 pulse.
We utilize the echo method which crops up in every corner of science and technology fields, including signal processing, NMR\cite{Jenkins}, acoustics\cite{Pal2018}, and laser physics\cite{Ma2021}.
...
Let $t_d=\pi/\Delta$. For a given function, the superposition $f(t)$, $f(t)+f(t-t_d)$'s Fourier transform evaluated at $\Delta$ is
$$
...
If we impose that $f(t)$ be also a flat-top pulse, then the pulse design problem boils down to designing a function
on $[0,t_r]$, where $t_r$ denotes the rise time.

By normalizing this rising function, the problem can be stated that we find a function $g(t)$ on $[-1,1]$ that satisfies
...
Note that for a function $f(t)$, $iz\mathcal{F}[f](z) = \mathcal{F}[f^\prime](z)$.
...
The derivative of the flat-top pulse in \ref{subsubsec:drag1_method} consists of two
symmetric functions $h(t)$ and $-h(t-T+t_r)$ as depicted in \fref{fig:drag2}.
$g^\prime(t)=h(t)$. $h(t)$ is a symmetric function since it is a derivative of a anti-symmetric function.
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
Then $\Omega_x$, $\Omega_y$, and $\delta$ can be expressed
\begin{equation}
...
\cite{Werschnik_2007}
A direct appraoch to applying \eref{eq:riccati_Omega} and \eref{eq:riccati_delta} involves initially determining the variables $x_1$ and $x_2$,
followed by a numerical solution for $\tilde{\Omega}$ and $\delta$.
...
Riccati engineering method.
From existing $\tilde{\Omega}$ and $\delta$, we numerically solve the corresponding $x_1$ and $x_2$,
then we add variations to thus obtained $x_1$ and $x_2$ to reduce leakage errors and obtain a higher fidelity operation.
...
Although the objective seems to be the same as in the case of $X$ gate,
we are more interested in mitigating the Fourier transform for an interval of frequencies \cite{Martinis2014}.
In the following we propose several methods to achieve this.
