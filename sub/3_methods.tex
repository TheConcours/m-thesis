...
approach builds on the DRAG-1 scheme which conventionally relies on a seed function to generate a control pulse to
minimize leakage from $\vert{}0\rangle$ to $\vert{}2\rangle$. This conventional method, however, has a disadvantage that
it's hard to control the resulting pulse shape. We establish a method to construct DRAG-1 pulse bypassing the seed function
method and synthesize a pulse which can achieve a shorter gate duration.
% \subsection{X gate}

\subsection{Reformulating DRAG-1}
...
$$
  \Omega_1^2(t) = \int_0^t\,\left[\dfrac{1}{\Delta_2}\sin(\Delta_2(T-\tau))\right][\Delta_2^2\Omega_x(\tau)^2]\,\dd \tau
$$
the DRAG-1 formula in integral form. In the same parlance, we call \eref{eq:drag1} differential DRAG-1 formula.
...
\subsection{Reformulating DRAG-2}
...
Having established that DRAG-1 and DRAG-2 formula are a differential way to
create pulses which has zero Fourier transform at $\Delta$ and $\Delta_3$ respectively,
we now utilize echo method to create pulses in signal processing, NMR, and laser physics.
